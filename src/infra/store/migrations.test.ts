...
describe("migrateSavedConfig — tmux-cc pass-through", () => {
  it("accepts a v1 tmux-cc config unchanged", () => {
    const raw = {
      id: "cfg-tmux",
      name: "tmux-saved",
      version: 1,
      type: "tmux-cc",
      config: { tmuxSessionName: "work", baseConfigId: "cfg-1" },
    };
    const result = migrateSavedConfig(raw);
    expect(result).toEqual({
      id: "cfg-tmux",
  ...
  it("accepts a v1 tmux-cc config without version field", () => {
    const raw = {
      id: "cfg-tmux-noversion",
      name: "tmux-noversion",
      type: "tmux-cc",
      config: { socketName: "dev" },
    };
    const result = migrateSavedConfig(raw);
    expect(result).not.toBeNull();
    expect(result?.type).toBe("tmux-cc");
    expect(result?.version).toBe(1);
    expect(result?.config).toEqual({ socketName: "dev" });
  });
});
