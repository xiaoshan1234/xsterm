...
| 显示与布局 | 终端尺寸模式 (Sizing Mode) | `enum` (auto, fixed) | `auto` | 控制终端如何响应容器尺寸变化。`auto` 通过 ResizeObserver 自动跟随；`fixed` 锁定到下方的 `cols` × `rows`，容器调整不影响终端。运行时切换模式通过 `EditSessionDialog` 保存即生效，无需重启会话。 |
| 显示与布局 | 列 (Columns) — Fixed 模式 | `number` | `80`（固定模式必填） | 仅在 `Sizing Mode === "fixed"` 时生效。 |
| 显示与布局 | 行 (Rows) — Fixed 模式 | `number` | `24`（固定模式必填） | 仅在 `Sizing Mode === "fixed"` 时生效。 |
