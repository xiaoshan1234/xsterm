...
前端 `SSHSessionConfig`（`src/types/session.ts:84-118`）和 spec 文档（`doc/history/prd-0.1-requirements/create-session-config.md:112-117`）都把认证字段定义为扁平结构：`auth_type: "password" | "key"` + 独立 `password` / `key_file` / `passphrase`。前端发送的 JSON 形如 `{ "auth_type": "password", "password": "..." }`，但后端期望 `{ "authType": "password", "password": "..." }` —— 缺少 `authType` tag，serde 直接抛错。
