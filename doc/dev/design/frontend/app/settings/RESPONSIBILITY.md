...
1. **持久化**——load / save / reset settings 到 `shared/service/persistence`
...
- **不实现 IPC 序列化**——settings 通过 `shared/service/persistence` 间接调 IPC
...
│   ├── apply/
│   │   ├── theme.ts                  applyTheme (dark/light/auto → service/theme)
│   │   ├── logLevel.ts               applyLogLevel → service/logger
...
| `shared/service` | settings 读 `shared/service/settings/store` 监听变化 |
