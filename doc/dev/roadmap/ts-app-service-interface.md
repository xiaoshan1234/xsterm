...
| OQ-3 | `service/*/actions.ts` 包装层去留                                                                                 | (a) 删（已知是死代码）；(b) 保留并强制 use case 通过它（包一层语义增强，例如自动 logger）；(c) 保留但不强制                                                                                            | (a) — 死代码不删会留坑                                                                                      | C        |
...
| OQ-5 | `Terminal.tsx` 的 keypress 直写 `writeSession`（I-12 入口）是否绕过 use case？                                           | 当前：Terminal.tsx 通过 `usePasteBatcher` 等 hook 间接调 `infra/tauri/commands/sessions.writeSession`（不走 use case）                                                       | **允许 UI hook 直调 infra IPC**（属于 I-12 的合法路径）；不走 use case 是因为 use case 会引入 store mutation 副作用，键入路径必须纯净 | ⏳ 待审     |
