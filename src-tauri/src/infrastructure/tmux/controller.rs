...
//! `spawn_dispatch_task`) so tests can drive them with mock I/O —
//! `Cursor<Vec<u8>>` for stdout, `tokio::io::duplex` for stdin/stdout
//! pipes, and a hand-rolled `RecordingBackend` for assertions — without
//! spawning a real tmux binary.
...
use crate::infrastructure::tmux::commands as tmux_cmd;
use crate::infrastructure::tmux::events::ControlEvent;
use crate::infrastructure::tmux::parser::ControlParser;
...
pub struct TmuxController {
    ...
    /// itself.
    pane_bindings: std::sync::Mutex<HashMap<String, u32>>,
    ...
    /// window id.
    pane_window_bindings: std::sync::Mutex<HashMap<String, String>>,
    ...
    /// Notified once the first pane has been registered; consumed by
    /// [`TmuxController::await_first_pane`].
    first_pane_notify: Notify,
    /// Stored alongside the notify so [`TmuxController::await_first_pane`]
    /// can return both the xsterm id and the tmux pane id of the first
    /// pane.
    first_pane_result: std::sync::Mutex<Option<(u32, String)>>,
    ...
    pending_splits: std::sync::Mutex<VecDeque<oneshot::Sender<SplitResult>>>,
    /// FIFO queue of [`oneshot::Sender`]s awaiting the result of a
    ...
    pending_windows: std::sync::Mutex<VecDeque<oneshot::Sender<NewWindowResult>>>,
    ...
    /// frontend).
    pending_window_pane: std::sync::Mutex<HashMap<String, PendingWindow>>,
    ...
    window_bindings: std::sync::Mutex<HashMap<String, u32>>,
...
struct PendingWindow {
    ...
    /// bootstrap window.
    sender: Option<oneshot::Sender<NewWindowResult>>,
}
...
impl TmuxController {
    ...
    fn spawn_with_backend(
        ...
        let controller = Arc::new(Self {
            ...
            first_pane_notify: Notify::new(),
            first_pane_result: std::sync::Mutex::new(None),
    ...
    pub async fn await_first_pane(&self) -> Result<(u32, String), String> {
        // Fast path: the dispatch task already ran and stored a result.
        ...
        match tokio::time::timeout(AWAIT_FIRST_PANE_TIMEOUT, self.first_pane_notify.notified())
            .await
        {
            Ok(()) => self
                .first_pane_result
                ...
                .ok_or_else(|| {
                    format!(
                        "tmux controller {}: first pane notify fired but no result stored",
                        self.controller_id
                    )
                }),
            Err(_) => Err(format!(
    ...
    pub(crate) fn record_first_pane(&self, xsterm_id: u32, pane_id: String) {
        let mut slot = match self.first_pane_result.lock() {
            Ok(m) => m,
            Err(_) => return,
        };
        if slot.is_none() {
            *slot = Some((xsterm_id, pane_id));
            self.first_pane_notify.notify_waiters();
        }
    }
...
mod tests {
    ...
    fn register_pane_idempotent_and_lookup_round_trip() {
        ...
        assert_eq!(
            controller.first_pane_result.lock().unwrap().clone(),
