...
/// (see `ssh.rs::SshSession::write`) and by every other async
...
impl SessionBackend for LocalSession {
    ...
    fn write(&self, bytes: &[u8]) -> Result<(), String> {
        ...
        self.writer_tx.try_send(bytes.to_vec()).map_err(|e| match e {
    ...
    fn close(mut self: Box<Self>) -> Result<(), String> {
        ...
        // `self.handles.pair` is dropped when the box is dropped, which on
