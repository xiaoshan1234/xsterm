...
//! ## History
//!
//! Originally `crate::services::tmux::escape`. Renamed to `codec` and moved
//! under `protocol/` in PR-T1 (see
...
/// [`crate::services::tmux::protocol::wire::send_keys`]. Callers that
/// need to embed arbitrary UTF-8 text should send it through
/// `escape_output` first so tmux's decoder sees the same `\nnn`
/// sequences we encode here.
