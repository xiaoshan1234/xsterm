...
//! strongly-typed [`ProtocolEvent`]s.
//!
//! `ProtocolParser` is deliberately **pure**: it has no I/O, no channels, no
...
//! returns `Option<ProtocolEvent>` for each line. This separation makes the
...
use super::codec::unescape_output;
use super::events::ProtocolEvent;

/// State machine for the tmux `-CC` line protocol.
///
/// Construct with [`ProtocolParser::new`] and call [`ProtocolParser::feed`]
...
impl ProtocolParser {
    ...
    ///   as keepalive pings).
    /// - `Some(ProtocolEvent::Unknown { line })` for unrecognised lines so
    ///   the controller can log them without crashing.
    /// - `Some(ProtocolEvent::{Output, CommandEnd, ...})` for recognised events.
    ///
    ...
    pub fn feed(&mut self, line: &str) -> Option<ProtocolEvent> {
    ...
    fn feed_inside_block(&mut self, line: &str) -> Option<ProtocolEvent> {
        ...
        match prefix {
            "end" if rest.len() == 3 => {
                ...
                if Some(id) == self.current_command_id {
                    ...
                    Some(ProtocolEvent::CommandOutput {
                        id: active_id,
                        line: line.to_string(),
                    })
                }
            ...
            "error" if rest.len() >= 3 => {
                ...
                if Some(id) == self.current_command_id {
                    self.current_command_id = None;
                    Some(ProtocolEvent::CommandError {
                        id,
                        timestamp,
                        flags,
                        message,
            ...
            _ => Some(ProtocolEvent::CommandOutput {
                id: active_id,
                line: line.to_string(),
            }),
        }
    ...
    fn parse_outer(&mut self, line: &str) -> Option<ProtocolEvent> {
        let stripped = line.strip_prefix('%').unwrap_or(line);
        let tokens: Vec<&str> = stripped.split_ascii_whitespace().collect();
        if tokens.is_empty() {
        ...
        match prefix {
            "begin" if rest.len() == 3 => match parse_begin_triple(rest) {
                Some((timestamp, id, flags)) => {
                    ...
                    Some(ProtocolEvent::CommandBegin {
                        id,
                        timestamp,
                        flags,
                    })
                }
                None => Some(ProtocolEvent::Unknown {
                    line: line.to_string(),
                }),
            },
            "begin" => Some(ProtocolEvent::Unknown {
                line: line.to_string(),
            }),

            "end" | "error" => Some(ProtocolEvent::Unknown {
                line: line.to_string(),
            }),
            ...
            "session-changed" => {
                split_after_first(rest).map(|(session_id, name)| ProtocolEvent::SessionChanged {
                    session_id: session_id.to_string(),
                    name: name.to_string(),
                })
            }
            "session-renamed" => {
                split_after_first(rest).map(|(session_id, name)| ProtocolEvent::SessionRenamed {
                    session_id: session_id.to_string(),
                    name: name.to_string(),
                })
            }
            "session-closed" => first_token(rest).map(|session_id| ProtocolEvent::SessionClosed {
                session_id: session_id.to_string(),
            }),
            "session-window-changed" => {
                first_two(rest).map(
                    |(session_id, window_id)| ProtocolEvent::SessionWindowChanged {
                        session_id: session_id.to_string(),
                        window_id: window_id.to_string(),
                    },
                )
            }
            "sessions-changed" => Some(ProtocolEvent::SessionsChanged),

            "window-add" => first_token(rest).map(|window_id| ProtocolEvent::WindowAdd {
                window_id: window_id.to_string(),
            }),
            "window-close" => first_token(rest).map(|window_id| ProtocolEvent::WindowClose {
                window_id: window_id.to_string(),
            }),
            "window-renamed" => {
                split_after_first(rest).map(|(window_id, name)| ProtocolEvent::WindowRenamed {
                    window_id: window_id.to_string(),
                    name: name.to_string(),
                })
            }
            "window-pane-changed" => {
                first_two(rest).map(|(window_id, pane_id)| ProtocolEvent::WindowPaneChanged {
                    window_id: window_id.to_string(),
                    pane_id: pane_id.to_string(),
                })
            }
            "unlinked-window-add" => {
                first_token(rest).map(|window_id| ProtocolEvent::UnlinkedWindowAdd {
                    window_id: window_id.to_string(),
                })
            }
            "unlinked-window-close" => {
                first_token(rest).map(|window_id| ProtocolEvent::UnlinkedWindowClose {
                    window_id: window_id.to_string(),
                })
            }
            ...
            "pane-died" => first_token(rest).map(|pane_id| ProtocolEvent::PaneDied {
                pane_id: pane_id.to_string(),
            }),

            "paste-buffer-changed" => Some(ProtocolEvent::PasteBufferChanged {
                buffer_name: rest.join(" "),
            }),
            "client-detached" => Some(ProtocolEvent::ClientDetached {
                client: rest.join(" "),
            }),
            "client-session-changed" => {
                // <client> <session_id> <name...>
                if rest.len() >= 3 {
                    Some(ProtocolEvent::ClientSessionChanged {
                        client: rest[0].to_string(),
                        session_id: rest[1].to_string(),
                        name: rest[2..].join(" "),
                    })
                } else {
                    Some(ProtocolEvent::Unknown {
                        line: line.to_string(),
                    })
                }
            }
            ...
            "config-error" => Some(ProtocolEvent::ConfigError {
                message: rest.join(" "),
            }),

            "pause" => first_token(rest).map(|pane_id| ProtocolEvent::Pause {
                pane_id: pane_id.to_string(),
            }),
            "continue" => first_token(rest).map(|pane_id| ProtocolEvent::Continue {
                pane_id: pane_id.to_string(),
            }),

            "popup-open" => Some(ProtocolEvent::PopupOpen {
                line: rest.join(" "),
            }),
            "popup-output" => Some(ProtocolEvent::PopupOutput {
                line: rest.join(" "),
            }),
            "popup-close" => Some(ProtocolEvent::PopupClose {
                line: rest.join(" "),
            }),

            _ => Some(ProtocolEvent::Unknown {
                line: line.to_string(),
            }),
        }
    }
...
fn parse_layout_change(line: &str, rest: &[&str]) -> Option<ProtocolEvent> {
    if rest.len() < 3 {
        return Some(ProtocolEvent::Unknown {
            line: line.to_string(),
        });
    }
    Some(ProtocolEvent::LayoutChange {
...
mod tests {
    ...
    fn feed_one(parser: &mut ProtocolParser, line: &str) -> ProtocolEvent {
    ...
    fn bare_percent_line_is_unknown() {
        let mut p = ProtocolParser::new();
        let ev = feed_one(&mut p, "%");
        assert_eq!(ev, ProtocolEvent::Unknown { line: "%".into() });
    }
    ...
    fn session_window_changed() {
        ...
        assert_eq!(
            ev,
            ProtocolEvent::SessionWindowChanged {
                session_id: "$2".into(),
                window_id: "@9".into()
            }
        );
    ...
    fn window_add_close_unlinked() {
        ...
        assert_eq!(
            ...
            ProtocolEvent::UnlinkedWindowAdd {
                window_id: "@3".into()
            }
        );
        assert_eq!(
            feed_one(&mut p, "%unlinked-window-close @4"),
            ProtocolEvent::UnlinkedWindowClose {
                window_id: "@4".into()
            }
        );
    }
    ...
    fn pane_mode_changed_and_pane_exited_and_pane_died() {
        ...
        assert_eq!(
            ...
            ProtocolEvent::PaneModeChanged {
                pane_id: "%3".into()
            }
        );
        assert_eq!(
            feed_one(&mut p, "%pane-exited %3"),
            ProtocolEvent::PaneExited {
                pane_id: "%3".into()
            }
        );
        assert_eq!(
            feed_one(&mut p, "%pane-died %3"),
            ProtocolEvent::PaneDied {
                pane_id: "%3".into()
            }
        );
    }
    ...
    fn paste_buffer_changed() {
        ...
        assert_eq!(
            ev,
            ProtocolEvent::PasteBufferChanged {
    ...
    fn client_session_changed_three_fields() {
        ...
        assert_eq!(
            ev,
            ProtocolEvent::ClientSessionChanged {
    ...
    fn exit_bare() {
        ...
        assert_eq!(ev, ProtocolEvent::Exit { reason: None });
    }
